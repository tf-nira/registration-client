 ...
 * 
 * import eu.gnome.morena.Device; import eu.gnome.morena.DeviceBase; import
 * eu.gnome.morena.TransferDoneListener;
 * 
 ...
 * BufferedImage image; int code; String error; boolean transferDone = false;
 * 
 * public void transferDone(File file) { if (file != null) try { image =
 * ImageIO.read(file); } catch (IOException e) { error =
 * e.getLocalizedMessage(); } notifyRequestor(); }
 * 
 * public void transferFailed(int code, String error) { this.code = code;
 * this.error = error; notifyRequestor(); }
 * 
 * private synchronized void notifyRequestor() { transferDone = true;
 * this.notify(); }
 * 
 * }
 */
     ...
     */
/*
 * static class FileTransferHandler implements TransferDoneListener { File
 * imageFile; int code; String error; boolean transferDone = false;
 */
        /**
         ...
         */
/*
 * public void transferDone(File file) { imageFile = file; notifyRequestor(); }
 */
 ...
 */
/*
 * public static BufferedImage scanImage(Device device, int item) throws
 * Exception { ImageTransferHandler th = new ImageTransferHandler();
 * 
 * synchronized (th) { ((DeviceBase) device).startTransfer(th, item); while
 * (!th.transferDone) th.wait(); } if (th.image != null) return th.image; throw
 * new Exception(th.error); }
 */
    /**
     * Convenient method that starts scanning process on specified device from
     ...
     */
/*
 * public static File scanFile(Device device) throws Exception { return
 * scanFile(device, 0); }
 */
    /**
     * Convenient method that starts scanning process on specified device from
 ...
 * 
 * synchronized (th) { ((DeviceBase) device).startTransfer(th, item); while
 * (!th.transferDone) th.wait(); } if (th.imageFile != null) return
 * th.imageFile; throw new Exception(th.error); } }
 */